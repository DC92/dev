...
function infos_points($conditions)
{
  ...
  $query_dom = "SELECT id_commentaire,id_point, id_createur_commentaire, auteur_commentaire
	FROM commentaires 
	LEFT JOIN phpbb3_users ON commentaires.id_createur_commentaire = phpbb3_users.user_id
	WHERE user_id IS NULL
		AND id_createur_commentaire >0
	";
  
  if ( ! ($res = $pdo->query($query_dom)))
    return erreur("Une erreur sur la requête est survenue",$query_dom);
  ...
  {
//   $use[$point->id_createur_commentaire]++;
if($point->auteur_commentaire)
   $use[$point->id_createur_commentaire]=$point->auteur_commentaire;
...
//*DCMM*/echo"<pre style='background-color:white;color:black;font-size:14px;'> = ".var_export($p,true).'</pre>';
  ...
  ksort($use);
  /*DCMM*/echo"<pre style='background-color:white;color:black;font-size:14px;'>====================================== = ".var_export($use,true).'</pre>';
